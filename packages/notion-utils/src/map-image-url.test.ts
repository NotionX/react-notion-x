...
const signedSource =
  'https://file.notion.com/f/f/space-id/file-id/image.png?table=block&id=block-id&spaceId=space-id&expirationTimestamp=1787061600000&signature=signature'
...
describe('defaultMapImageUrl', () => {
  test('uses current signed Notion file URLs directly', () => {
    expect(defaultMapImageUrl(signedSource, imageBlock)).toBe(signedSource)
  })
  ...
  test('uses signed legacy S3 URLs directly', () => {
    const signedLegacySource = `${legacySource}?X-Amz-Credential=credential&X-Amz-Signature=signature`

    expect(defaultMapImageUrl(signedLegacySource, imageBlock)).toBe(
      signedLegacySource
    )
  })
  ...
  test('falls back to the Notion proxy for unsigned legacy file URLs', () => {
    ...
    expect(mappedUrl.origin).toBe('https://www.notion.so')
  ...
  test('uses Notion static image paths directly', () => {
    expect(
      ...
      'https://www.notion.so/images/page-cover/met_william_morris_1877.jpg'
    )
  })
})
...
describe('Notion file URL helpers', () => {
  ...
  ])('recognizes private Notion file URL %s', (url) => {
